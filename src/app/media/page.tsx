...
const THREE_D_ASSETS: ThreeDAsset[] = [
  ...
  { id: "ast-3", name: "Cinematic Orbit Cam", category: "camera", icon: "", color: "bg-[#D92A2A]" },
...
export default function MediaStudioPage() {
  ...
  const [stageObjects, setStageObjects] = useState<Array<{ id: string; name: string; x: number; y: number; color: string; icon: string }>>([
    { id: "obj-1", name: "Cinematic Orbit Cam", x: 25, y: 35, color: "bg-[#D92A2A]", icon: "" },
  ...
  return (
    <div className="h-full bg-[#09090B] flex flex-col overflow-hidden">
      ...
      <header className="px-6 py-5 border-b border-[#27272A] shrink-0 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold text-white">Visual Content & Media Studio</h1>
          <p className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest mt-0.5">
            Dedicated Workflows: Movies, Shorts, Faceless YouTube Channels & 3D Video Creator
          </p>
        </div>

        {/* Studio Mode Selector */}
        <div className="flex items-center gap-1 bg-[#111113] border border-[#27272A] p-1.5 rounded-xl flex-wrap">
          <button
            onClick={() => setActiveStudio("movies")}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeStudio === "movies" ? "bg-[#D92A2A] text-white shadow-lg" : "text-[#A1A1AA] hover:text-white"
            }`}
          >
             Movies & Longs
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeStudio === "shorts" ? "bg-[#D92A2A] text-white shadow-lg" : "text-[#A1A1AA] hover:text-white"
            }`}
          >
             Shorts & Reels
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeStudio === "faceless-yt" ? "bg-[#D92A2A] text-white shadow-lg" : "text-[#A1A1AA] hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeStudio === "3d-creator" ? "bg-[#D92A2A] text-white shadow-lg" : "text-[#A1A1AA] hover:text-white"
            }`}
          >
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#09090B]">
        ...
        {activeStudio === "movies" && (
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="bg-[#111113] border border-[#27272A] rounded-xl p-5 flex items-center justify-between">
              <div>
            ...
            <div className="bg-[#111113] border border-[#27272A] rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-[#27272A] pb-3">
                <span className="text-xs font-semibold text-white">16:9 Cinematic Video Preview Player</span>
                <span className="text-xs font-mono text-[#D92A2A]">● LIVE MONITOR (60 FPS)</span>
              </div>
              ...
              <div className="relative aspect-video w-full bg-[#09090B] border border-[#27272A] rounded-xl overflow-hidden shadow-2xl group">
                ...
                <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-md border border-white/10 rounded-lg p-3 text-[11px] font-mono text-white">
                  <div className="text-[#D92A2A] font-bold">CINEMATIC MOVIE ENGINE</div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest block">Cinematic Scene Prompt</label>
                <textarea
                  value={moviePrompt}
                  onChange={e => setMoviePrompt(e.target.value)}
                  className="w-full bg-[#09090B] border border-[#27272A] rounded-xl p-4 text-xs text-white placeholder-[#71717A] outline-none focus:border-[#D92A2A]/40 transition-all min-h-[70px] resize-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-6 items-start">
              ...
              <div className="md:col-span-1 bg-[#111113] border border-[#27272A] rounded-xl p-5 space-y-4">
                <h3 className="text-xs font-semibold text-white">Shorts Script & Voiceover</h3>
                <textarea
                  value={shortsPrompt}
                  onChange={e => setShortsPrompt(e.target.value)}
                  className="w-full bg-[#09090B] border border-[#27272A] rounded-xl p-3 text-xs text-white outline-none focus:border-[#D92A2A]/40 min-h-[120px] resize-none"
                />
                
                <div className="space-y-2">
                  <label className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest block">Caption Style</label>
                  <select
                    value={captionStyle}
              ...
              <div className="md:col-span-2 bg-[#111113] border border-[#27272A] rounded-xl p-6 flex flex-col items-center justify-center">
                <span className="text-xs font-semibold text-white mb-4">Dedicated 9:16 Vertical Video Player</span>
                ...
                <div className="relative w-[280px] h-[500px] bg-[#09090B] border-2 border-[#27272A] rounded-3xl overflow-hidden shadow-2xl group">
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-3 bg-gradient-to-t from-black/90 to-transparent flex items-center justify-between text-white text-xs">
                    <button 
                      ...
                      className="w-7 h-7 rounded-full bg-[#D92A2A] flex items-center justify-center font-bold"
                    >
                      {shortsIsPlaying ? "" : "▶"}
                    </button>
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="bg-[#111113] border border-[#27272A] rounded-xl p-5 flex items-center justify-between">
              ...
              <button 
                ...
                className="px-5 py-2.5 bg-[#D92A2A] text-white text-xs font-semibold rounded-xl hover:bg-[#D92A2A]/90 transition-all disabled:opacity-40 flex items-center gap-2"
              >
                {ytIsGenerating ? (
                  <>
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-all ${
                      isSelected 
                        ? "bg-[#111113] border-[#D92A2A] shadow-lg" 
                        : "bg-[#09090B] border-[#27272A] hover:border-[#27272A]/80"
                    }`}
                  >
                    <span className="text-[10px] font-mono text-[#D92A2A] uppercase block mb-1">Preset Channel</span>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {/* Left Config Panel */}
              <div className="md:col-span-1 bg-[#111113] border border-[#27272A] rounded-xl p-5 space-y-4">
                <h3 className="text-xs font-semibold text-white">Channel Setup & Script Engine</h3>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest block">AI Voice Narrator</label>
                  <select
                    value={ytVoice}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest block">B-Roll Footage Source</label>
                  <select
                    value={ytBroll}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#71717A] uppercase tracking-widest block">YouTube Script</label>
                  <textarea
                    value={ytScript}
                    onChange={e => setYtScript(e.target.value)}
                    className="w-full bg-[#09090B] border border-[#27272A] rounded-xl p-3 text-xs text-white outline-none focus:border-[#D92A2A]/40 min-h-[140px] resize-none"
                  />
                </div>
              </div>

              {/* Center: DEDICATED FACELESS YOUTUBE VIDEO PLAYER MONITOR */}
              <div className="md:col-span-2 bg-[#111113] border border-[#27272A] rounded-xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-[#27272A] pb-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#D92A2A] animate-pulse" />
                    <span className="text-xs font-semibold text-white">Dedicated Faceless YouTube Video Player</span>
                  </div>
                ...
                <div className="relative aspect-video w-full bg-[#09090B] border border-[#27272A] rounded-xl overflow-hidden shadow-2xl">
                  {ytIsGenerating ? (
                    <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center space-y-3">
                      <div className="w-10 h-10 border-2 border-[#D92A2A] border-t-transparent rounded-full animate-spin" />
                      <p className="text-xs font-mono text-[#D92A2A] font-bold">SYNTHESIZING VOICE & MATCHING B-ROLL...</p>
                    </div>
                  ) : (
                    <video
                ...
                <div className="p-4 bg-[#09090B] border border-[#27272A] rounded-xl space-y-2">
                  ...
                  <div className="h-6 bg-[#111113] rounded-lg border border-[#27272A] flex items-center px-3 gap-1 overflow-hidden">
                    {Array.from({ length: 40 }).map((_, i) => (
                      <div
                        key={i}
                        className="w-1 bg-[#D92A2A] rounded-full transition-all"
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="bg-[#111113] border border-[#27272A] rounded-xl p-5 flex items-center justify-between">
              ...
              <button 
                ...
                className="px-4 py-2 bg-[#D92A2A] text-white text-xs font-semibold rounded-xl hover:bg-[#D92A2A]/90 transition-all flex items-center gap-2"
              >
            ...
            <div className="grid md:grid-cols-4 gap-6">
              {/* Left: 3D Asset Shelf */}
              <div className="md:col-span-1 bg-[#111113] border border-[#27272A] rounded-xl p-5 space-y-4">
                <h3 className="text-xs font-semibold text-white">3D Asset Palette (Drag from here)</h3>
                ...
                <div className="space-y-2">
                  {THREE_D_ASSETS.map(asset => (
                    <div
                      ...
                      className="flex items-center gap-3 p-3 bg-[#09090B] border border-[#27272A] rounded-xl cursor-grab active:cursor-grabbing hover:border-[#D92A2A]/40 transition-all group"
                    >
                      <span className={`w-7 h-7 rounded-lg ${asset.color} flex items-center justify-center text-xs text-white`}>
                        {asset.icon}
                      </span>
                      <div>
                        <p className="text-xs font-semibold text-white group-hover:text-[#D92A2A] transition-colors">{asset.name}</p>
              ...
              <div className="md:col-span-3 bg-[#111113] border border-[#27272A] rounded-xl p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-white">Interactive 3D Stage & Render Viewport</span>
                ...
                >
                  {/* 3D Grid Overlay */}
                  <div className="absolute inset-0 opacity-20 pointer-events-none bg-[radial-gradient(#D92A2A_1px,transparent_1px)] [background-size:16px_16px]" />
                  ...
                  {renderProgress !== null && renderProgress < 100 && (
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-md z-30 flex flex-col items-center justify-center text-white space-y-3">
                      <div className="w-12 h-12 border-4 border-[#D92A2A] border-t-transparent rounded-full animate-spin" />
                      <p className="text-xs font-mono font-bold text-[#D92A2A]">RENDERING 3D SCENE VIDEO... {renderProgress}%</p>
                    </div>
                  )}
                  ...
                  {renderProgress === 100 && (
                    <div className="absolute inset-0 z-30 bg-black/90 flex flex-col items-center justify-center p-4">
                      ...
                      <button 
                        ...
                        className="absolute top-4 right-4 bg-[#D92A2A] text-white text-xs px-3 py-1.5 rounded-lg"
                      >
                        Close Video Player
                      </button>
                    </div>
                    ...
                    return (
                      <div
                        ...
                        className={`absolute -translate-x-1/2 -translate-y-1/2 p-3 rounded-xl border-2 cursor-pointer transition-transform hover:scale-110 flex items-center gap-2 shadow-2xl ${
                          isSelected ? "border-[#D92A2A] bg-[#D92A2A]/20 text-white scale-105" : "border-[#27272A] bg-[#111113] text-[#A1A1AA]"
                        }`}
                      >
                        <span className="text-base">{obj.icon}</span>
                        <span className="text-xs font-semibold text-white whitespace-nowrap">{obj.name}</span>
                      </div>
                    );
                  })}
                ...
                <div className="bg-[#09090B] border border-[#27272A] rounded-xl p-4 space-y-2">
                  ...
                  <div className="h-8 bg-[#111113] border border-[#27272A] rounded-lg relative overflow-hidden flex items-center px-3">
                    <div className="h-full bg-[#D92A2A]/20 border-r-2 border-[#D92A2A] w-[40%]" />
                    <span className="absolute left-3 text-[10px] font-mono text-[#D92A2A] font-bold">Keyframe 120 (Camera Orbit Sweep)</span>
                  </div>
                </div>
              </div>
            </div>
